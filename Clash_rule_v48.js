const PROVIDER_URLS = {
  "机场1": "在此替换为你的第1个机场订阅链接",
  "机场2": "在此替换为你的第2个机场订阅链接",
  "机场3": "在此替换为你的第3个机场订阅链接",
};
...
function main(params) {
    ...
    const customProviders = buildProviders(PROVIDER_URLS);
    if (Object.keys(customProviders).length > 0) {
        params["proxy-providers"] = Object.assign({}, params["proxy-providers"], customProviders);
    }
...
function buildProviders(urls) {
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        const tag = (Array.isArray(v) && v[1]) ? String(v[1]) : "";
        
        if (!url || String(url).trim().charAt(0) === "#" || String(url).indexOf("在此替换") !== -1) {
            continue;
        }
